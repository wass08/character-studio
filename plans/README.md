...
4 sub-plans shipped end-to-end (verification pass done 2026-05-25). 1 in-progress: engine rewrite — **TS layer 100% complete (9/9 files), TSL/WebGPU pending hardware**. 1 newly opened: Playground polish (first per-experiment sub-plan under experiences-polish). Two pick-up paths, ordered by readiness:

1. **Experiences — Playground polish Phase 1** — sub-plan opened 2026-05-25 at [app-experiences-playground.md](app-experiences-playground.md). Phase 1 has three independent sub-slices: (a) backdrop presets in [Backdrop.tsx](../src/components/scene/Backdrop.tsx) + [Scene.tsx](../src/components/scene/Scene.tsx) + a store field + a UI switcher, (b) mobile bottom-sheet variant of [PhotoGalleryBox.jsx](../src/components/ui/PhotoGalleryBox/PhotoGalleryBox.jsx), (c) copy-link per gallery thumb. Any order; each is a small commit. Phase 1a (presets) is the most opinion-heavy slice — lock the preset roster (strawman: `studio` / `sunset` / `night`) in the plan body before coding. Lipsync + Platformer sub-plans still pending — open via `/start-plan` when picked up.
2. **Engine rewrite Phase 1b (WebGPU spike on Backdrop)** — only with hardware. Convert Backdrop's floor material to TSL, add the renderer-switch factory in Scene.tsx behind a feature flag, eyeball Chrome (WebGPU) + Safari (WebGL fallback), measure perf + bundle delta. The TS scaffolding (Scene.tsx as the renderer-switch home, `useCharacter()` as the data API) is in place. See [app-engine-rewrite.md `### Phases 1b–7b`](app-engine-rewrite.md#phases-1b7b-tslwebgpu).
...
| 1.6 | [Experiences polish](app-experiences-polish.md) | draft (Phase 1 ✓) | p3 | **Phase 1 specs landed 2026-05-25** — one-paragraph shippable spec per experiment with markers + smallest-change set. Lipsync + Platformer sub-plans open lazily via `/start-plan` as they're picked up. |
| 1.6.1 | [Experiences — Playground](app-experiences-playground.md) | draft (ready) | p3 | **Opened 2026-05-25** — first per-experiment polish sub-plan. Phase 1 = backdrop presets + mobile gallery sheet + copy-link per photo (the parent spec's "smallest unblock-launch change set"). Three independent sub-slices, any order. |
