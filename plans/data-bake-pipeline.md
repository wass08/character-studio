...
- 2026-09-04 regression found after the above: every 1.0.0 bake rendered nothing because per-mesh quantization volumes plus the worker's skin consolidation scattered the meshes ~25 m away (see `bake-worker/README.md`). Fixed in pipeline 1.1.0 (scene-wide quantization volume, collapsed placeholder plane, separate vertex layout), covered by a golden test that skins a synthetic asset and checks bind-pose bounds, and verified by rendering the bake headlessly in the app. Baked wall slots also needed the Rig turn removed to match the live wall orientation.
